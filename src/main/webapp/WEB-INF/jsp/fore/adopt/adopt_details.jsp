...
<title>HeMaozhu-领养文章-文章页面</title>
			...
			<div class="col-lg-10 col-lg-offset-1">
				<div class="ibox">
					<div class="ibox-content">
						...
						<div class="text-center article-title"><h1>${adopt.title}</h1></div>
						<div class="hr-line-dashed"></div>
						<div class="row">
							<div class="col-sm-7">
								<c:if test="${empty pet.photo}">
									<img src="http://fpoimg.com/500x500?text=No IMG Was Uploaded" class="img-responsive" style="width:500px;float: right;"/>
								</c:if>
								<c:if test="${not empty pet.photo}">
									<img src="${ctx}${pet.photo}" class="img-responsive" style="width:500px;float: right;">
								</c:if>
                            </div>
							<div class="col-sm-5">
                            	<h2><strong>名字：</strong> ${pet.petname}</h2>
                            	<h2><strong>品种：</strong> ${pet.variety}</h2>
                            	<h2><strong>性别：</strong> 
                            		<c:if test="${pet.gendar==0}">公</c:if>
                                    <c:if test="${pet.gendar==1}">母</c:if>
                                </h2>
                            	<h2><strong>年龄：</strong> ${pet.age}</h2>
                            	<h2><strong>生日：</strong> ${pet.birthday}</h2>
                            </div>
						</div>
						<div class="hr-line-dashed"></div>
						${adopt.content}
