...
<body class="gray-bg">
    ...
    <script type="text/javascript">
    	...
    	function load(page){
    		...
    		$.ajax({
	            url: '${ctx}/fore/cityadopt/minedata/' + userId,
	            type: 'get',
	            data: {
	            	current: page, 	 // 当前页 1
	            	size: 5,      // 一页显示多少 10
	            ...
	            success : function(data){
	            	...
	            	if (rows.length > 0) {
	                    for (var i = 0; i < rows.length; i++) {
	                    	blogList += "<div class='ibox'><div class='ibox-content'><a href='javascript:void(0);' onclick=\"openTabPage('${ctx}/admin/cityadopt/update/"
	                    	+ rows[i].cityadoptId + "', '" + rows[i].title + "')\" class='btn-link'><h2>"
	                    	...
	                    	+ verify(rows[i].verify) + "" + status(rows[i].cityadoptId, rows[i].status) + "</div></div></div></div></div>";
	                    }
	                }else{
    	...
    	function status(id, value) {
            ...
            + id + "', '0')\">待领养</a></li><li><a href='javascript:void(0);' onclick=\"changestatus('"
            + id + "', '1')\">领养中</a></li><li><a href='javascript:void(0);' onclick=\"changestatus('"
            + id + "', '2')\">已领养</a></li><li class='divider'></li><li><a href='javascript:void(0);' onclick=\"del('"
			+ id + "')\">删除</a></li></ul></div>";
			return result;
		}
		...
		function del(id) {
			window.parent.layer.confirm("确认删除?", {icon: 3, offset: 't'}, function () {
				$.ajax({
                    url: '${ctx}/admin/cityadopt/delete/' + id,
                    type: 'get',
                    success: function (response) {
                        if (response.code == 0) {
                            window.parent.layer.msg(response.msg, {icon: 1, time: 1000, offset: 't'}, function(){
