...
<title>HeMaozhu-管理系统-物资入库</title>
					...
					<div class="ibox-content">
						<form class="form-horizontal m-t" onsubmit="return false"
							...
							<shiro:hasRole name="站点管理员">
								<div class="form-group">
								<label class="col-sm-4 control-label">总部仓库对应物资名：</label>
								<div class="col-sm-5">
									<select id="parentId" class="chosen-select" name="parentId" style="width: 120px;">
										<option value="-1">请选择物资名称</option>
										<c:forEach items="${pgoodslist}" var="pgoods">
											<option value="${pgoods.goodsId}">${pgoods.name}</option>
										</c:forEach>
									</select>
								</div>
							</div>
