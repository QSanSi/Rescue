...
<body class="gray-bg">
	...
	<script>
		...
		function edit(id) {
			var title = "修改信息";
			var url = "${ctx}/fore/users/update/" + id;
			layer.open({
				type : 2,
				title : title,
				shadeClose : true,
	    ...
	    $('#btn-delete').click(function() {
	        ...
	        if (rows.length == 0) {
	            ...
	            window.parent.layer.confirm("确认删除?", {icon: 3, offset: 't'}, function () {
	                $.ajax({
	                    url: '${ctx}/fore/users/delete/' + rows[0].userId,
	                    type: 'get',
	            ...
	            window.parent.layer.confirm("确认批量删除?", {icon: 3, offset: 't'}, function () {
	                ...
	                $.ajax({
	                    url: '${ctx}/fore/users/deletebatch',
		...
		var list_url = '${ctx}/fore/users/data';
		// 初始化表格数据
