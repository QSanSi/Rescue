...
public class CommonExceptionHandler {
	 ...
	 * handlerAuthenticationException(AuthenticationException e, HttpServletRequest
	 * request) { logger.error("用户名或密码错误", e); Map<String, Object> reponseData = new
	 * HashMap<>(); reponseData.put("code", 1); reponseData.put("msg",
	 * "用户名或密码错误");//错误的详细信息 reponseData.put("error", "用户名或密码错误"); return
	 * modelAndView(request, reponseData); }
	 */
	    ...
	    private ModelAndView modelAndView(HttpServletRequest request, Map<String, Object> responseData) {
	        ...
	        if (isAjaxRequest(request)) {
	            ...
	            mv.setViewName("admin/error"); // 跳转的error.jsp 页面  //WEB-INF/jsp/error.jsp
	        }
	        return mv;
	    }
