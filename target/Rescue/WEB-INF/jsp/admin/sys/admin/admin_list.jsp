...
<body class="gray-bg">
	...
	<script>
		function addFunctionAlty(value, data, index) {
			var id = data.userId;
			var result = "";
			result += "<button id='assign-role' class='btn btn-primary' onclick=\"assign('"
					+ id
					+ "')\"><i class='fa fa-user-plus'></i> 分配角色</button>&nbsp;&nbsp;";
		...
		function emailFormatter(value, data, index) {
			return "<a href='mailto:" + value + "' title='单击打开链接'>" + value
					+ "</a>";
		}
		// 查询	
		...
		function assign(id) {
			var title = "分配角色";
			var url = "${ctx}/sys/admin/assign/role/" + id;
			layer.open({
				type : 2,
				title : title,
				shadeClose : true,
	    ...
	    $('#btn-delete').click(function() {
	        ...
	        if (rows.length == 0) {
	            ...
	            window.parent.layer.confirm("确认删除?", {icon: 3, offset: 't'}, function () {
	                $.ajax({
	                    url: '${ctx}/sys/admin/delete/' + rows[0].userId,
	                    type: 'get',
	                    success: function (response) {
	                        if (response.code == 0) {
	                            window.parent.layer.msg(response.msg, {icon: 1, time: 1000, offset: 't'}, function(){
		...
		var list_url = '${ctx}/sys/admin/data';
		// 初始化表格数据
