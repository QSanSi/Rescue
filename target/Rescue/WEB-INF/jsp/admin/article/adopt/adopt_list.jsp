...
<title>HeMaozhu-领养文章-文章列表</title>
...
<body class="gray-bg">
	...
	var list_url = '${ctx}/admin/adopt/data';
	// 初始化表格数据
	var dataTable = $('#data-table').bootstrapTable({
		url : list_url, //  请求后台的URL
		method : "get", //  请求方式
		uniqueId : "adoptId", //  每一行的唯一标识，一般为主键列
	...
	function info(id) {
		var title = "详细信息";
		var url = "${ctx}/fore/adopt/find/" + id;
		layer.open({
			type : 2,
			title : title,
			shadeClose : true,
	...
	function edit(id) {
		...
		var url = "${ctx}/admin/adopt/update/" + id + "/" + deptId;
		layer.open({
			type : 2,
			title : title,
			shadeClose : true,
	...
	$('#btn-add').click(function() {
		var title = "新增";
		var deptId = $('#deptId').val()
		var url = "${ctx}/admin/adopt/add/" + deptId;
		layer.open({
			type : 2,
			title : title,
			shadeClose : true,
    ...
    $('#btn-delete').click(function() {
        ...
        if (rows.length == 0) {
            ...
            window.parent.layer.confirm("确认删除?", {icon: 3, offset: 't'}, function () {
                $.ajax({
                    url: '${ctx}/admin/adopt/delete/' + rows[0].adoptId,
                    type: 'get',
            ...
            window.parent.layer.confirm("确认批量删除?", {icon: 3, offset: 't'}, function () {
                ...
                for (var i = 0; i < rows.length; i++) {
                    ids.push(rows[i].adoptId);
                }
                $.ajax({
                    url: '${ctx}/admin/adopt/deletebatch',
