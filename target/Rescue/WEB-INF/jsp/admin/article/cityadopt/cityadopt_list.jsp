...
<body class="gray-bg">
	...
	var list_url = '${ctx}/admin/cityadopt/data';
	// 初始化表格数据
	var dataTable = $('#data-table').bootstrapTable({
		url : list_url, //  请求后台的URL
		method : "get", //  请求方式
		uniqueId : "cityadoptId", //  每一行的唯一标识，一般为主键列
	...
	function info(id) {
		var title = "详细信息";
		var url = "${ctx}/fore/cityadopt/find/" + id;
		layer.open({
			type : 2,
			title : title,
			shadeClose : true,
	...
	function edit(id) {
		var title = "修改信息";
		var url = "${ctx}/admin/cityadopt/update/" + id;
		layer.open({
			type : 2,
			title : title,
			shadeClose : true,
        ...
        if (rows.length == 0) {
        	...
        	if(rows[0].verify != 0){
        		...
        		window.parent.layer.confirm("确认通过?", {icon: 3, offset: 't'}, function () {
                    $.ajax({
                        url: '${ctx}/admin/cityadopt/accept/' + rows[0].cityadoptId,
                        type: 'get',
            ...
            if(b){
                window.parent.layer.confirm("确认通过全部所选项?", {icon: 3, offset: 't'}, function () {
                    $.ajax({
                        url: '${ctx}/admin/cityadopt/acceptbatch',
    ...
    $('#btn-delete').click(function() {
        ...
        if (rows.length == 0) {
            ...
            window.parent.layer.confirm("确认删除?", {icon: 3, offset: 't'}, function () {
                $.ajax({
                    url: '${ctx}/admin/cityadopt/delete/' + rows[0].cityadoptId,
                    type: 'get',
                    success: function (response) {
                        if (response.code == 0) {
                            window.parent.layer.msg(response.msg, {icon: 1, time: 1000, offset: 't'}, function(){
            ...
            window.parent.layer.confirm("确认批量删除?", {icon: 3, offset: 't'}, function () {
                ...
                $.ajax({
                    url: '${ctx}/admin/cityadopt/deletebatch',
                    contentType: "application/json; charset=UTF-8",//发送给服务器的是json数据
                    type: 'post',
                    dateType: 'json',
                    data: JSON.stringify(ids),
